				...
				unsigned long attrs)
{
	...
	if (!plat_device_is_coherent(dev) && !(attrs & DMA_ATTR_SKIP_CPU_SYNC))
		__dma_sync_virtual(dma_to_virt(dev, daddr), size, dir);
	mb();
...
{
	if (!plat_device_is_coherent(dev) && !(attrs & DMA_ATTR_SKIP_CPU_SYNC))
		__dma_sync_virtual(dma_to_virt(dev, dev_addr), size, dir);
				...
				unsigned long attrs)
{
	...
	if (!plat_device_is_coherent(dev) && !(attrs & DMA_ATTR_SKIP_CPU_SYNC)) {
		for_each_sg(sgl, sg, nents, i)
			__dma_sync_virtual(dma_to_virt(dev, sg->dma_address), sg->length, dir);
	}
	mb();
				...
				enum dma_data_direction dir)
{
	...
	if (!plat_device_is_coherent(dev))
		__dma_sync_virtual(dma_to_virt(dev, dma_handle), size, dir);
	mb();
}
				...
				enum dma_data_direction dir)
{
	...
	if (!plat_device_is_coherent(dev)) {
		for_each_sg(sgl, sg, nents, i) {
			__dma_sync_virtual(dma_to_virt(dev,
